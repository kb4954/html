...
rollback;--마지막 커밋했던 시점으로 롤백됨.

--한꺼번에 하는 경우에는 total , avg 점수가 바뀌지않음.
...
--평균이 80점 이상이면서 국어점수가 90점 이상인 사람을 출력하시오
--select 뒤에는 컬럼을 작성하기
select * from stuscore where avg>=80 and kor>=90;

--이안에는 평균이 80점이상인 사람이 들어가있음
select sno,kor,avg from atuscore where avg>=80;
--위에것을 임시테이블로 생각하여 괄호 안에 넣기
...
select * from stuscore3;
select sno, name,total,kor,eng,math,avg,rank from stuscore;
desc stuscore3;
...
-- 비어있는 rank 칸 채우기
--1. 별칭 설정
select kor+eng as ke from stuscore3 s; -- 별칭 생성시 as 생략 가능
select total from stuscore3 order by total desc; -- 점수로 높은순으로 정렬
select sno, rank from stuscore3;--아래랑 위치점이 달라서 바로 넣을 수 없음. 따라서 찾아서 넣어햐함
...
select sno, rank() over( order by total desc) ranks from stuscore3; --ranks 별칭 생성

select ranks from(select sno, rank() over( order by total desc) ranks from stuscore3)b --ranks 별칭 생성
where a.sno = b.sno;
update stuscore3 a
set rank = (
...
select min(total) form stuscore3; --최소값
select count(total) form stuscore3; --개수
select avg(total) from stuscore3; -- 평균:224.4
select avg(avg) from stuscore3; -- 74.8
select sum(total) from stuscore3;


--전체 평균보다 이하인 학생들에게만 전체평균값을 입력하시오.
update stuscore3
...
--문자열은 사칙연산 불가능 근데 날짜는 가능

--근무일수에서 +100일후 보너스 지급 할 예정 출력
select hire_date, hire_date+100 h3 from employees order by h3 asc;
select hire_date from employees
where hire_date<='04-01-01';

select * from employees;
--월급 4000이상 이면서 부서번호가 50이상인 사원을 검색하시오.
select * from employees where 4000<=salary and employee_id=50 order by employee_id;


--사원번호 150번 이상이거나 입사일이 2007/01/01 이전인 사원을 검색하시오.
select employee_id, emp_name,hire_date from employees where employee_id>=150 or hire_date <='2007/01/01' order by employee_id asc;  

--월급 4000,6000,7000인 사원을 검색하시오
select emp_name, salary from employees where salary =4000 or salary = 6000 or salary = 7000 order by salary asc;
...
select manager_id from employees where manager_is null;


--부서번호가 50번인 사원을 검색하시오.
select department_id, emp_name from employees where department_id = 50 order by department_id asc;

--부서번호가 20,30번이 아닌 사원을 검색하시오.
select department_id, emp_name from employees where department_id != 20 and department_id !=30 order by department_id asc;
...
select * from employees;
-- 월급이 3800이상 이면서 sh_clerk 인 사람 찾기
select * from employees where job_id='SH_CLERK' and salary>=3800 order by salary desc;
...
--이름중에 n이 들어가있는 사원을 출력하시오 
select emp_name from employees where emp_name like'%n%';

--이름중에 2번째 o가 들어가 있는 사원이름을 검색하시오.
select emp_name from employees where emp_name like '_o%';
select emp_name from employees where emp_name like '__u%'; -- _를문자로 인식하게 하는것 \

select * from stuscore;
--이름중에 순인 사람을 검색하시오
select * from stuscore where name like '%순%';

--2번째 a가 들어가있는 사람을 찾으시오 
select name from stuscore where name like '_a%';
select emp_name, job_id from employees where job_id like '%#_%' escape'#'; -- _를 찾을때

--입사일이 00/01/01~07/12/31 등가로도 표현 가능
...
-- 날짜 25년 이후부터 3월까지 에 가입한 회원을 출력하시오
...
-- ↑ lower로 이름을 다 소문자로 바꾸고 검색하면 대문자 이름도 찾아짐

-- salary,연봉 1430원, 한화, 천단위 표시 -> 3번째에 g가 들어가 있는 사원만 출력하시오. 대소문자 구분없이 


select salary,salary*12,salary+salary*nvl(commission_pct,0), -- 월급, 연봉에 해당
salary *12*1438,to_char(salary*12*1438,'999,999,999')--원화로 환산 및 천단위로 환산
from employees
...
select email from employees where email like '%E%' or email like'%S%';

select * from stuscore;

--2개 order by 정렬, 1차 우선 정렬 후 같은 값일 경우, 2차정렬방식을 적용 2차 값이 같을때 3차 값을 적용
select * from stuscore order by kor asc, eng desc, math desc; -- math 이미 앞쪽에 있는 정렬을 따르기 때문에 순서가 안맞아 보일 수 있음 


select * form stuscore;
